...
/**
 * The seven states a session row can be in.
 *
...
export type SessionStatus =
  ...
  | 'working'
  /**
   * Stuck on something that is not you: a rate limit, an API outage, a compaction that never came
   * back. Nothing to answer — but not finished either, and worth seeing.
   */
  | 'stalled'
...
export const SESSION_STATUSES: readonly SessionStatus[] =
  ['approval', 'question', 'finished', 'working', 'stalled', 'seen', 'dormant'];
 ...
 */
export const API_RETRY_WINDOW_MS = 5 * 60_000;

/** How long a compaction is given to finish before it is treated as a stall. */
export const COMPACTION_WINDOW_MS = 5 * 60_000;
...
export interface JsonlRecord {
  ...
  /**
   * Set by Claude Code on a synthetic assistant record standing in for a failed API call — a rate
   * limit, an overload, a DNS failure. See {@link isApiError}.
   */
  isApiErrorMessage?: boolean;
  /** Present on the record Claude Code writes when it compacts a conversation. */
  isCompactSummary?: boolean;
  compactMetadata?: unknown;
  message?: {
    content?: string | ContentBlock[];
    /** `<synthetic>` on records Claude Code fabricated rather than received from the API. */
    model?: string;
  };
}
 ...
 */
export function isApiError(record: JsonlRecord): boolean {
 ...
 */
export function isCompaction(record: JsonlRecord): boolean {
...
export function claudeStatusFromTail(
  ...
  for (let i = records.length - 1; i >= 0; i--) {
    ...
    if (isApiError(record)) {
      return quietMs < API_RETRY_WINDOW_MS ? 'working' : 'stalled';
    }

    if (isCompaction(record)) {
      // Compaction writes nothing while it runs and then resumes on its own.
      return quietMs < COMPACTION_WINDOW_MS ? 'working' : 'stalled';
    }
...
export interface DisplayStatusInput {
  ...
  /**
   * What the plugin's own hooks observed about this session, if it runs them — a prompt seen open, a
   * turn seen ending, the session seen closing. See `src/hookActivity.ts`.
   *
   * Applied *before* the `pending` upgrade so a live host read still wins: the host is looking at
   * the running process, while this is a file written a moment ago.
   */
  hookState?: HookStateInput;
...
/**
 * The subset of `HookSessionState` this module needs. Declared here rather than imported so
 * `sessionStatus.ts` keeps its one-way dependency direction — `hookActivity.ts` imports from here.
 */
export interface HookStateInput {
  pending?: 'approval' | 'question';
  idle?: boolean;
  settled?: boolean;
}
...
export function resolveDisplayStatus(
  ...
  if (withHooks !== 'finished') { return withHooks; }
 ...
 *     blocked state.
 */
export function foldHookState(
  base: SessionStatus, state: HookStateInput | undefined,
): SessionStatus {
  ...
  if (state.idle && base === 'working') { return 'finished'; }
  return base;
}
